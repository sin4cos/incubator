...
#define WINDOW_TITLE "camera example"
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // 摄像机位置
        glm::vec3 camera_pos = glm::vec3(0.0f, 0.0f, 3.0f);     // 不要忘记正z轴是从屏幕指向你的，如果我们希望摄像机向后移动，我们就往z轴正方向移动。

        // 摄像机方向
        glm::vec3 camera_target = glm::vec3(0.0f, 0.0f, 0.0f);  // 我们想要摄像机指向场景原点(0, 0, 0)
        glm::vec3 camera_direction = glm::normalize(camera_pos - camera_target);    // 让摄像机位置坐标去减摄像机目标位置坐标就可以得到从摄像机目标到摄像机的方向向量。注意它正好指向从它到目标向量的相反方向。

        // 右向量（它代表摄像机空间的x轴的正方向）
        glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f);     // 上向量
        glm::vec3 camera_right = glm::normalize(glm::cross(up, camera_direction));      // 上向量与摄像机放量叉乘，即可得到摄像机的右向量

        // 上轴
        glm::vec3 camera_up = glm::cross(camera_direction, camera_right);   // 有了摄像机的x轴和z轴后，将两者叉乘即可得到摄像机的yu轴，也就是上轴

        // 摄像机的 lookat 矩阵
        // glm::LookAt 函数需要一个摄像机位置坐标、摄像机目标位置坐标和摄像机的上向量。
        GLfloat radius = 10.0f;     // 本示例中，摄像机按半径为10的圆形轨迹绕目标原点旋转
        GLfloat cam_x = sin(glfwGetTime()) * radius;
        GLfloat cam_z = cos(glfwGetTime()) * radius;
        glm::mat4 view = glm::mat4(1.0f);
        view = glm::lookAt(glm::vec3(cam_x, 0.0f, cam_z),   // 摄像机额位置坐标
                           glm::vec3(0.0f, 0.0f, 0.0f),     // 摄像机所指向的目标的位置坐标
                           glm::vec3(0.0f, 1.0f, 0.0f));    // 摄像机的上向量（本示例的摄像机的上向量正好是 y 轴的正方向）
